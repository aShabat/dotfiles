...
vim.b.miniai_config = vim.tbl_deep_extend('force', MiniAi.config, { search_method = 'nearest' })
set(']c', function()
...
set('<leater>rr', '<CMD>MoltenReevaluateCell<CR>', '[R]unner [R]eevaluate')
set('<leader>rmd', '<CMD>MoltenDelete<CR>', '[R]unner [M]olten [D]elete')
set('<leader>rR', '<CMD>MoltenRestart!<CR>', '[R]unner [R]estart')
set('<leader>oh', '<CMD>MoltenHideOutput<CR>', '[O]utput [H]ide')

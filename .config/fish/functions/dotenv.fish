...
function dotenv
    ...
    if set -q _flag_help
        echo "Usage: loadenv [OPTIONS] [COMMAND ...]"
        echo ""
        echo "Run command with .env variables"
        echo ""
        echo "Options:"
        echo "  --help, -h      Show this help message"
        echo "  --verbose, -v   Print env values before execution"
        echo "  --file, -f      File with env values. Default: .env"
        echo ""
        echo "Arguments:"
    ...
    while test $lineNumber -lt $totalLines
        ...
        set --function --export $key $value
        if set -q _flag_verbose; or test (builtin count $argv) -eq 0
            echo "$key=$value"
        end
    end
